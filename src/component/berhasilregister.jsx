...
const BerhasilRegister = ({ popUp, setPopupRegister }) => {
  if (!popUp) return null; // Jika popup tidak aktif, jangan render apa pun

  const handleClose = () => {
    setPopupRegister(false); // Menutup popup
  };
  ...
  return (
    <>
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-lg p-6 w-80 text-center z-20">
        ...
        <p className="text-lg font-semibold text-gray-800 mb-6">Berhasil Register</p>
        <button
          onClick={handleClose}
...
export default BerhasilRegister;
